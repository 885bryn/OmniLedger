...
- [x] **ITEM-01**: User can create an item with type `RealEstate`, `Vehicle`, `FinancialCommitment`, or `FinancialIncome`.
...
- [x] **ITEM-05**: User can request `GET /items/:id/net-status` and receive the item with attributes plus nested linked child commitments.
...
- [x] **DEPL-01**: User can run API and PostgreSQL together with `docker-compose.yml` for local network hosting.
